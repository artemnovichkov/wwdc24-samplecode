...
Abstract:
Metal shaders used to create a G-buffer.
*/
...
// interpolated by the rasterizer and fed to each fragment generated by clip-space primitives.
typedef struct
{
                                 ...
                                 constant AAPLFrameData  & frameData  [[ buffer(AAPLBufferFrameData) ]])
{
    ...
    // Make the position a `float4` to perform 4 x 4 matrix math on it.
                                      ...
                                      depth2d<float>           shadowMap    [[ texture(AAPLTextureIndexShadow) ]])
{
    ...
    // frame of reference. If the sample is occluded, the value is zero.
