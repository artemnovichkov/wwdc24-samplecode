...
class ComplicationController: NSObject, CLKComplicationDataSource {
    ...
    func privacyBehavior(
        for complication: CLKComplication
    ) async -> CLKComplicationPrivacyBehavior {
    ...
    func complicationDescriptors() async -> [CLKComplicationDescriptor] {
        [
            CLKComplicationDescriptor(
                identifier: "Coffee_Tracker_Caffeine_Dose",
    ...
    func timelineEntries(
        ...
        // Create a timeline entry for every 5 minutes from the starting time.
    ...
    func localizableSampleTemplate(
        ...
        // Because it's more than 24 hours in the future,
        // our template always shows zero cups and zero mg of caffeine.
...
extension ComplicationController {
    ...
    private func createTimelineEntry(
        forComplication complication: CLKComplication, date: Date
    ...
    private func createTemplate(
        ...
        case .modularSmall:
            createModularSmallTemplate(forDate: date)
        case .modularLarge:
            createModularLargeTemplate(forDate: date)
        case .utilitarianSmall, .utilitarianSmallFlat:
            createUtilitarianSmallFlatTemplate(forDate: date)
        case .utilitarianLarge:
            createUtilitarianLargeTemplate(forDate: date)
        case .circularSmall:
            createCircularSmallTemplate(forDate: date)
        case .extraLarge:
            createExtraLargeTemplate(forDate: date)
        case .graphicCorner:
            createGraphicCornerTemplate(forDate: date)
        case .graphicCircular:
            createGraphicCircleTemplate(forDate: date)
        case .graphicRectangular:
            createGraphicRectangularTemplate(forDate: date)
        case .graphicBezel:
            createGraphicBezelTemplate(forDate: date)
        case .graphicExtraLarge:
            createGraphicExtraLargeTemplate(forDate: date)
        @unknown default:
    ...
    private func createModularSmallTemplate(forDate date: Date) -> CLKComplicationTemplate {
        ...
        return CLKComplicationTemplateModularSmallStackText(
            line1TextProvider: mgCaffeineProvider,
    ...
    private func createModularLargeTemplate(forDate date: Date) -> CLKComplicationTemplate {
        ...
        let mgCaffeineProvider = CLKSimpleTextProvider(text: data.mgCaffeineString(atDate: date))
        let mgUnitProvider = CLKSimpleTextProvider(text: "mg Caffeine", shortText: "mg")
        let combinedMGProvider = CLKTextProvider(
            format: "%@ %@", mgCaffeineProvider, mgUnitProvider)
        ...
        let combinedCupsProvider = CLKTextProvider(
            format: "%@ %@", numberOfCupsProvider, cupsUnitProvider)

        // Create the template using the providers.
        let imageProvider = CLKImageProvider(
            onePieceImage: #imageLiteral(resourceName: "CoffeeModularLarge"))
    ...
    private func createUtilitarianSmallFlatTemplate(forDate date: Date) -> CLKComplicationTemplate {
        // Create the data providers.
        let flatUtilitarianImageProvider = CLKImageProvider(
            onePieceImage: #imageLiteral(resourceName: "CoffeeSmallFlat"))
        ...
        return CLKComplicationTemplateUtilitarianSmallFlat(
            textProvider: combinedMGProvider,
    ...
    private func createGraphicRectangularTemplate(forDate date: Date) -> CLKComplicationTemplate {
        ...
        let imageProvider = CLKFullColorImageProvider(
            fullColorImage: #imageLiteral(resourceName: "CoffeeGraphicRectangular"))
        let titleTextProvider = CLKSimpleTextProvider(text: "Coffee Tracker", shortText: "Coffee")
    ...
    private func createGraphicBezelTemplate(forDate date: Date) -> CLKComplicationTemplate {
        ...
        let circle = CLKComplicationTemplateGraphicCircularImage(
            imageProvider: CLKFullColorImageProvider(
                fullColorImage: #imageLiteral(resourceName: "CoffeeGraphicCircular")))
